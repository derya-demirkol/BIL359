...
$res = $conn->query("SELECT * FROM kitaplar WHERE id=$kitap_id");
...
if($_SERVER['REQUEST_METHOD'] == 'POST'){
    ...
    if($baslik && $yazar && $yayin_yili){
        $sql = "UPDATE kitaplar SET baslik='$baslik', yazar='$yazar', yayin_yili='$yayin_yili' WHERE id=$kitap_id";
        if($conn->query($sql)){
            header("Location: kitaplar.php");
            exit();
        } else {
...
<title>Eser Güncelle - Admin</title>
...
<main id="content">
<h2>Eser Güncelle (ID: <?= $kitap_id ?>)</h2>
