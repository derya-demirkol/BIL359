...
if($_SERVER['REQUEST_METHOD'] == 'POST'){
    ...
    $sql_parcasi = "UPDATE kullanicilar SET kulad='$kulad', rol='$rol', durum='$durum'";
    
    if(!empty($yeni_sifre)){
        $hashli_sifre = hash('sha256', $yeni_sifre);
        $sql_parcasi .= ", sifre='$hashli_sifre'";
    }
    
    $sql_parcasi .= " WHERE id=$id";
    
    if($conn->query($sql_parcasi)){
        header("Location: uyeler.php");
        exit();
    } else {
...
<div class="form-card">
    <form method="post">
        ...
        <input type="password" name="sifre" id="sifre" placeholder="Değiştirmek istemiyorsanız boş bırakın">
